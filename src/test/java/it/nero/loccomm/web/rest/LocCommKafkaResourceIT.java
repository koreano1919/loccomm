...
import it.nero.loccomm.config.KafkaProperties;
...
class LocCommKafkaResourceIT {
    ...
    void setup() {
        ...
        LocCommKafkaResource kafkaResource = new LocCommKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/loc-comm-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/loc-comm-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
